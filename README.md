...
```json
{
  ...
  "unmatched": [{ "raw_name": "string", "context": "short string", "candidate_fight_ids": ["string"] }]
}
```
...
- React/Vite/Tailwind client and Hono Worker in one project.
- Workers Static Assets routing, `/health`, strict TypeScript, linting and formatting.
- Local D1, first migration and repository layer.
- Vitest and Playwright harnesses.
- CI build/test workflow and protected Cloudflare preview/prod environment configuration.
- Root `AGENTS.md` and ignored local-secret/cache files.
- Deployed spike measuring free-plan CPU for representative validation/fuzzy matching.
- Cloudflare Access proof for production and preview origins.

Exit gate:

- Local and CI builds pass; preview is authenticated; `/health` is public; preview cannot access production D1.
...
- Card/capper/source CRUD.
- Manual card and fully editable bout list.
- Responsive application shell and navigation.
- Seed command for the known cappers and source roles; do not create the stats tracker as an individual capper.
...
- LLM provider interface and Anthropic/OpenRouter adapters.
- Versioned prompt files and Zod schemas.
- ExtractionRun lifecycle and source hashing.
- Individual, aggregator and stats-tracker parsing.
- Review UI for opinions, tips, fighter aliases and capper attribution.
- Chunking, retry, cost metadata and readable failures.
- Golden fixtures and mocked-provider tests.

Exit gate:

- All extraction acceptance fixtures pass; an identical reparse does not double-count; ambiguous identities require review.
...
- Typed synthesis configuration.
- Pure consensus, badge, tiering, conflict and allocation modules.
- SynthesisRun input/config snapshots.
- Full boundary and property-test suite.
- Deterministic preliminary summaries independent of LLM text.

Exit gate:

- Crown-jewel maths tests pass, shuffled input gives identical output, and every allocation invariant holds.
...
- Tracker-versus-capper presentation and dissent expansion.
- Batched overview/rationale generation with fallbacks.
- Draft/accept/supersede synthesis UI.
- Slate list with exposure and qualification explanations.
...
- Bet, BetLeg and BetSupport persistence.
- Place/skip/edit actions and actual odds/stakes.
- Manual bet and structured parlay builder.
- Recommended-versus-actual exposure display and over-budget confirmation.
- Immutable link back to the originating synthesis run.

Exit gate:

- Generated, manual and parlay bets survive reload; re-synthesis cannot mutate them; mobile placement flow passes E2E.
...
- Bookmaker-result settlement and explicit unsettle audit.
- Bankroll chart, card table and tier/market/origin splits.
- Capper winner/method/round grading.
- Exportable application backup.
...
- Structured-data event parser and LLM text fallback.
- Review diff and non-destructive re-fetch merge.
- Saved real-page fixtures; optional non-blocking live smoke.
- Observability, safe logs, rate limits and failure dashboards.
- Expand/contract migration and rollback runbook.
- Production deployment, authenticated synthetic check and restore rehearsal.
...
1. YouTube caption compatibility spike and fetch/fallback flow.
2. Historical capper weighting with no look-ahead.
3. Odds provider integration and CLV snapshots.
4. Calibration and extraction-quality reporting.

---

## 16. Tunable configuration
